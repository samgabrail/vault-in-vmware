...
resource "aws_iam_role_policy" "vault_backup" {
  count  = var.user_supplied_iam_role_name != null ? 0 : 1
  name   = "${var.resource_name_prefix}-vault-backup"
  role   = aws_iam_role.instance_role[0].id
...
resource "aws_iam_role_policy" "private_ca_access" {
  count  = var.user_supplied_iam_role_name != null ? 0 : 1
  name   = "${var.resource_name_prefix}-private-ca"
  role   = aws_iam_role.instance_role[0].id
  policy = data.aws_iam_policy_document.private_ca_access.json
}

data "aws_iam_policy_document" "private_ca_access" {
  statement {
    effect = "Allow"
